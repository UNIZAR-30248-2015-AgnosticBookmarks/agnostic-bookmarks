...
describe("Auth API", function(){
    ...
    it("should return 403 when user does not exist", function() {
        var result = {};
        var timeout = 7500;
        var done = false;
        runs(function() {
            request.post({url:baseURL + "api/bookmarkSearch", body:"{username: nouser}"},
    ...
    it("should return 403 when user does not have bookmarks", function() {
        var result = {};
        var timeout = 7500;
        var done = false;
        runs(function() {
            request.post({url:baseURL + "/api/bookmarkSearch", body:"{username: userEmpy}"},
                function(error, response, body) {
                    result.err = error;
                    result.response = response;
                    result.body = body;
